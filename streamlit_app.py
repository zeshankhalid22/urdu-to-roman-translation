...
class AttentionDecoder(nn.Module):
    ...
    def forward(self, input, hidden, cell, encoder_outputs):
        ...
        if input.dim() > 1 and input.shape[1] > 1:
            input = input[:, -1]  # Take the last token if it's a sequence
        
        if input.dim() == 1:
            input = input.unsqueeze(0)  # Add batch dimension if necessary
            
        embedded = self.dropout(self.embedding(input))
        embedded = embedded.unsqueeze(1) if embedded.dim() == 2 else embedded
        
        # Get attention weights using the top layer of the hidden state
        top_hidden = hidden[-1]
...
class Seq2Seq(nn.Module):
    ...
    def beam_search(self, src, beam_width=3, max_len=350):
        ...
        eos_idx = 2  # End token
        device = src.device  # Get the device from the source tensor
        ...
        input_token = torch.tensor([sos_idx], device=device).unsqueeze(0)

        # Initialize beam
        # Each beam contains: (cumulative_score, sequence, current_input, hidden, cell, token_history)
        beams = [(0, [sos_idx], input_token, hidden, cell, set())]
        completed_beams = []
        ...
        for _ in range(max_len):
            ...
            for score, seq, inp, h, c, history in beams:
                ...
                for i in range(beam_width):
                    ...
                    new_input = torch.tensor([token], device=device).unsqueeze(0)
        ...
        output = torch.zeros(1, max_len, dtype=torch.long, device=device)
...
def translate_with_beam_search(text, model, urdu_merges, urdu_vocab_map, roman_vocab_map):
    try:
        ...
        for idx in output[0, 1:].cpu().numpy():  # Skip <sos>
            if idx == 0 or idx >= len(roman_vocab_map):  # Skip padding and unknown
                continue
                
            token = None
            for k, v in roman_vocab_map.items():
                if v == idx:
                    token = k
                    break
        ...
        # Join tokens
        roman_text = ''.join(translation).replace('▁', ' ')
        return roman_text
    except Exception as e:
